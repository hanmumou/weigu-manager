<template>
  <div class="goodsmanage-box">
    <el-form :model="goodsForm"  label-width="120px" hide-required-asterisk="true">
      ...
      <el-form-item label="商品限购量：" required>
...
export default {
  ...
  methods: {
    ...
    handleRemove(file, fileList) {
      // 文件列表移除
      //this.dialogImageUrl = this.dialogImageUrl.splice(this.remove(this.dialogImageUrl, file.url), 1)
      for(let i=0;i<this.dialogImageUrl.length;i++){
         if(file.url == this.dialogImageUrl[i].url){
    ...
    async onSubmit(val) {
      ...
      if(this.dialogImageUrl.length > 0){
        for(let i=0;i<this.dialogImageUrl.length;i++){
          let imgs = {'image':''}
          imgs.image  = this.dialogImageUrl[i].url
          images.push(imgs)
        }
      }
      val.detail_picture = JSON.stringify(images)

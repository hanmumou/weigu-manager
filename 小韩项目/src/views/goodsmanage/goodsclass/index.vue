	...
	<div class="add">
		...
		<div class="add_table">
			<el-table :data="newlist" style="width: 100%" border>
				...
				<el-table-column prop="sort" label="分类排序" min-widt h="20%" align="center">
...
<script>
	...
	export default {
		...
		methods: {
			...
			async addshop() {
				console.log('添加列表')
				this.$store.dispatch('addshoplist', {
					value: this.value,
					sort: this.sort
				}).then(() => {
			...
			// 弹框的确定按钮，进行编辑
			async editlist(id, name, sort) {
				id = this.editRow.id
				name = this.form.name
				sort = this.form.sort
				try {
					//修改 商品分类
					const res = await editshop(id, name, sort)
					if(res.status == 200) {
						this.$message({
							message: '修改成功',
							type: 'success'
